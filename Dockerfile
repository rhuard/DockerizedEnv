...
RUN apt update && apt install -y dialog apt-utils && apt upgrade && apt install -y git \
    llvm clang build-essential python2.7 python3 python-pip gcc bcc cmake vim vim-nox-py2 wget curl \
    python-dev python3-dev tmux zsh ack-grep curl ranger pylint cowsay figlet exuberant-ctags colordiff diffuse \
    source-highlight dict dict-moby-thesaurus aspell feh bsdmainutils iputils-ping openjdk-8-jdk openjdk-8-jre
